 ...
 *   Rating: 1
 */
int isTmax(int x) { return !(((1 << 31) + x) + 1); }
/*
...
int isAsciiDigit(int x) {
  // 1. check first 24 bits
  int flag1 = !(x >> 8);
  // 2. check 0x3
  int flag2 = !((x >> 4) ^ 0x3);
  // 3. check between 0 to 9
  int flag3 = ((x & 0xf) + (~0xA + 1)) >> 31;
...
int isLessOrEqual(int x, int y) {
  // 1. both positive, x - y
  // 2. both negative, x - y
  // 3. differnt sign, just compare sign
  int sign_x = x >> 31;
  int sign_y = y >> 31;
